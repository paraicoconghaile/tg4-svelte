...
export async function getGenre(genre: string) {
    ...
    //const response = await fetch(`https://playerapi.tg4tech.com/genres?genre=${genre}`,
        {
            headers: {
...
export async function getSeriesVideos(seriesTitle: string) {
    ...
    /* console.log('Series Title:-', `${encodeURIComponent(seriesTitle)}`);
...
export async function getVideo(videoId: string) {
    //const url = `https://playerapi.tg4tech.com/video?id=${videoId}`;
